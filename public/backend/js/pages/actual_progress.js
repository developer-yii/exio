$(document).ready(function () {
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.status) {
                    ...
                    $(formId).find(".date").val(result.data.date);
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.status) {
                    ...
                    $(viewModalId).find(".date").html(result.data.date);
                    $(viewModalId)
                        .find(".status")
                    ...
                    if (result.data.images && result.data.images.length > 0) {
                        result.data.images.forEach(function (image) {
                            images += `<img src="${assetUrl}storage/actual_progress_images/${image.image}"
                          class="img-thumbnail"
                          style="width: 100px; height: 100px; margin-right: 10px; object-fit: cover;"
                          alt="Project Image">`;
                        });
                    }
                    $(viewModalId).find(".images").html(images);
                    $(viewModalId)
                        .find(".created_at")
    ...
    let listTable = $(tableId).DataTable({
        ...
        ajax: {
            type: "GET",
            url: apiUrl + "?project_id=" + actualProjectId,
            data: function (d) {
