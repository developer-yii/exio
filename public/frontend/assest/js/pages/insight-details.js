const reraDataPoints = [{ x: 0, y: 0 }, ...reraProgress.map(item => ({ x: item.work_completed, y: item.timeline }))];
const actualDataPoints = [{ x: 0, y: 0 }, ...actualProgress.map(item => ({ x: item.work_completed, y: item.timeline }))];

// Find the maximum timeline value in both datasets
const maxWorkCompleted = Math.max(
  ...reraDataPoints.map(d => d.x),
  ...actualDataPoints.map(d => d.x),
  100
);

// Generate y-axis tick values (every 6 months)
const yTicks = [];
  ...
  options: {
    ...
    plugins: {
      tooltip: {
        callbacks: {
          title: (tooltipItems) => {
            const value = tooltipItems[0].raw.y;
            return formatTimeline(value);
          },
          label: (tooltipItem) => {
            return `Work Completed: ${tooltipItem.raw.x}%`;
          }
        }
      },
      legend: {
